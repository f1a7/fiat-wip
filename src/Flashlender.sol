...
import {getCreditLine} from "./CDM.sol";

import {wmul} from "./utils/Math.sol";

/// @title Flashlender
/// @notice `Flashlender` enables flashlender minting / borrowing of FIAT and internal Credit
...
contract Flashlender is IFlashlender, ReentrancyGuard {
    ...
    constructor(IMinter minter_, uint256 protocolFee_)  {
        ...
        IFIAT fiat_ = fiat = minter_.fiat();
        protocolFee = protocolFee_;

        cdm_.modifyPermission(address(minter_), true);
        fiat_.approve(address(minter_), type(uint256).max);
    }
    ...
    /// @notice Flashlender lends `token` (FIAT) to `receiver`
    /// @dev Reverts if `Flashlender` gets reentered in the same transaction or if token is not FIAT
    ...
    function flashLoan(
        ...
        // reverts if not enough FIAT have been send back
        fiat.transferFrom(address(receiver), address(this), total);
