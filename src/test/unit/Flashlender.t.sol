...
import {wmul, WAD} from "../../utils/Math.sol";
...
abstract contract TestReceiver is FlashLoanReceiverBase {
    ...
    function _mintFiatFee(uint256 amount) internal {
        if (amount > 0) IFIAT(flashlender.fiat()).mint(address(this), amount);

    }

    function _mintCreditFee(uint256 amount) internal {
        if (amount > 0) {
            IFIAT(flashlender.fiat()).mint(address(this), amount);
            IFIAT(flashlender.fiat()).approve(address(flashlender.minter()), amount);
...
contract TestDEXTradeReceiver is TestReceiver {
    ...
    function onFlashLoan(
        ...
        // Perform a "trade"
        fiat.transfer(address(0x1), amount_);
...
contract FlashlenderTest is TestBase {
    ...
    function createCore() internal override {
        cdm = new TestCDM();
        fiat = new FIAT();
        minter = new Minter(cdm, fiat, address(this), address(this));
        ...
        fiat.grantRole(MINTER_AND_BURNER_ROLE, address(minter));
    ...
    function setUp() public override {
        ...
        // grant permissions
        fiat.grantRole(MINTER_AND_BURNER_ROLE, address(badReturn));

        fiat.grantRole(MINTER_AND_BURNER_ROLE, address(immediatePaybackReceiver));
        fiat.grantRole(MINTER_AND_BURNER_ROLE, address(immediatePaybackReceiverOne));
        fiat.grantRole(MINTER_AND_BURNER_ROLE, address(immediatePaybackReceiverFive));
    ...
    function test_flashloan_payback_zero_fees() public {
        ...
        flashlender.flashLoan(noFeePaybackReceiver, address(fiat), 10 ether, "");
    }
    ...
    function test_mint_payback_zero_fees() public {
        ...
        uint256 expectedFee = flashlender.flashFee(address(fiat), flashLoanAmount);

        // assert zero fee
        assertEq(expectedFee, 0);

        flashlender.creditFlashLoan(immediatePaybackReceiver, flashLoanAmount, "");
        flashlender.flashLoan(immediatePaybackReceiver, address(fiat), flashLoanAmount, "");
    ...
    function test_mint_payback_low_fee() public {
        uint256 flashLoanAmount = 10 ether;
        uint256 expectedFee = flashlenderOne.flashFee(address(fiat), flashLoanAmount);

        // assert fee is 1%
        assertEq(expectedFee, 10 ether * 1e16 / 1 ether);

        flashlenderOne.creditFlashLoan(immediatePaybackReceiverOne, flashLoanAmount, "");
        flashlenderOne.flashLoan(immediatePaybackReceiverOne, address(fiat), flashLoanAmount, "");
    ...
    function test_mint_payback_high_fee() public {
        uint256 flashLoanAmount = 10 ether;
        uint256 expectedFee = flashlenderFive.flashFee(address(fiat), flashLoanAmount);

        // assert fee is 5%
        assertEq(expectedFee, 10 ether * 5e16 / 1 ether);

        flashlenderFive.creditFlashLoan(immediatePaybackReceiverFive, flashLoanAmount, "");
        flashlenderFive.flashLoan(immediatePaybackReceiverFive, address(fiat), flashLoanAmount, "");
    ...
    function test_mint_zero_amount() public {
        ...
        flashlender.flashLoan(immediatePaybackReceiver, address(fiat), 0, "");
    }
    ...
    function test_mint_amount_over_max1() public {
        ...
        uint256 amount = flashlender.maxFlashLoan(address(fiat)) + 1 ether;
    ...
    function test_mint_amount_over_max2() public {
        ...
        uint256 amount = flashlender.maxFlashLoan(address(fiat)) + 1 ether;
        vm.expectRevert(CDM.CDM__modifyBalance_debtCeilingExceeded.selector);
        flashlender.flashLoan(immediatePaybackReceiver, address(fiat), amount, "");
    }
    ...
    function test_mint_max_zero2() public {
        cdm.setParameter(address(flashlender), "debtCeiling", 0);
        vm.expectRevert(CDM.CDM__modifyBalance_debtCeilingExceeded.selector);
        flashlender.flashLoan(immediatePaybackReceiver, address(fiat), 10 ether, "");
    }
    ...
    function test_mint_reentrancy2() public {
        vm.expectRevert("ReentrancyGuard: reentrant call");
        flashlender.flashLoan(reentrancyReceiver, address(fiat), 100 ether, "");
    }
    ...
    function test_dex_trade() public {
        ...
        flashlender.flashLoan(dexTradeReceiver, address(fiat), 100 ether, "");
    }

    function test_max_flash_loan() public {
        assertEq(flashlender.maxFlashLoan(address(fiat)), uint256(type(int256).max));
    ...
    function test_flash_fee() public {
        assertEq(flashlender.flashFee(address(fiat), 100 ether), 0);
        assertEq(flashlenderOne.flashFee(address(fiat), 100 ether), 1 ether);
        assertEq(flashlenderFive.flashFee(address(fiat), 100 ether), 5 ether);
    }
    ...
    function test_bad_return_hash2() public {
        ...
        flashlender.flashLoan(badReturn, address(fiat), 100 ether, "");
    }
    ...
    function test_no_callbacks2() public {
        vm.expectRevert();
        flashlender.flashLoan(IERC3156FlashBorrower(address(noCallbacks)), address(fiat), 100 ether, "");
    }
}