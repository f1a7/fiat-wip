...
import {ICDPVault, ICDPVaultBase, CDPVaultParams, CDPVaultConfigs} from "../../interfaces/ICDPVault.sol";
...
import {CDPVault, calculateDebt, calculateNormalDebt, EPOCH_DURATION, EPOCH_FIX_DELAY, EPOCH_FIX_TIMEOUT, VAULT_CONFIG_ROLE, TICK_MANAGER_ROLE} from "../../CDPVault.sol";
...
contract CDPVaultWrapper is CDPVault {
    ...
    function calculateRateAccumulator(GlobalIRS memory globalIRS) public view returns(uint64) {
        return _calculateRateAccumulator(globalIRS);
    }
...
contract VaultWrapperFactory {
    ...
    function getParams() view external returns (
        ICDM cdm_,
        IOracle oracle_,
        IBuffer buffer_,
        IERC20 token_,
...
contract CDPVaultTest is TestBase {
    ...
    function _calculateUtilizationBasedInterestRate(
        ...
        uint256 utilizationRatio = (totalDebt_ == 0)
            ? 0 : wdiv(totalDebt_, totalDebt_ + getCreditLine(balance, debtCeiling));
    ...
    function test_setParameter() public {
        ...
        vault.setParameter("baseRate", uint256(uint64(BASE_RATE_1_005)));
        ...
        assertEq(globalIRS.baseRate, BASE_RATE_1_005);
    ...
    function test_claimUndelegatedCredit_simple_delegateFirst() public {
        ...
        vm.warp(block.timestamp + EPOCH_DURATION * EPOCH_FIX_DELAY);
    ...
    function test_claimUndelegatedCredit_fixTimeout() public {
        ...
        vm.warp(block.timestamp + EPOCH_DURATION * (EPOCH_FIX_TIMEOUT));
        assertEq(vault.claimUndelegatedCredit(epoch), 100 ether);

        vm.revertTo(snapshot);

        vm.warp(block.timestamp + EPOCH_DURATION * (EPOCH_FIX_TIMEOUT + 1));
    ...
    function test_modifyCollateralAndDebt_revertsOnMaxUtilizationRatio() public {
        ...
        CDPVaultParams memory vaultParams = _getDefaultVaultParams();
        vaultParams.maxUtilizationRatio = 0.80 ether;

        CDPVaultParams_TypeA memory vaultParams_TypeA = _getDefaultVaultParams_TypeA();
        vaultParams_TypeA.targetHealthFactor = 1.15 ether;

        CDPVaultConfigs memory vaultConfigs = _getDefaultVaultConfigs();
        ...
        vm.expectRevert(CDPVault.CDPVault__checkUtilizationRatio_maxUtilizationRatio.selector);
    ...
    function test_exchange_multipleTicks() public {
        ...
        vault.deposit(address(this), 400 ether);
        address positionA = address(new PositionOwner(vault));
        // emit log_named_address("PositionA", positionA);
        vault.modifyCollateralAndDebt(positionA, address(this), address(this), 100 ether, 50 ether);
        address positionB = address(new PositionOwner(vault));
        // emit log_named_address("PositionB", positionB);
        vault.modifyCollateralAndDebt(positionB, address(this), address(this), 100 ether, 50 ether);
        address positionC = address(new PositionOwner(vault));
        // emit log_named_address("PositionC", positionC);
        vault.modifyCollateralAndDebt(positionC, address(this), address(this), 100 ether, 50 ether);
        address positionD = address(new PositionOwner(vault));
        // emit log_named_address("PositionD", positionD);
        ...
        vm.expectRevert();
        vault.exchange(WAD, 75 ether);

        vault.exchange(1.01 ether, 65 ether);
        assertEq(credit(address(this)), 135 ether);
        assertEq(debt(address(vault)), 135 ether);
        assertEq(vault.cash(address(this)), 50 ether + wdiv(uint256(15 ether), uint256(1.01 ether)));
        assertEq(credit(address(vault)), 0);
    }
