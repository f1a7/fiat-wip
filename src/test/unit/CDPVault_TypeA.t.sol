...
contract CDPVault_TypeATest is TestBase {
    ...
    function test_liquidatePositions_revertOnSafePosition() public {
        CDPVault_TypeA vault = createCDPVault_TypeA(token, 150 ether, 0, 1.25 ether, 1 ether, 1 ether, 1.05 ether, 0, WAD, int64(uint64(WAD)), 0, 0);
        // create position
    ...
    function test_reserve_liquidate_partial_1c() public {
        CDPVault_TypeA vault = createCDPVault_TypeA(token, 150 ether, 0, 1.25 ether, 1 ether, 0.95 ether, 1.05 ether, 0, WAD, int64(uint64(WAD)), 0, 0);
    ...
    function test_reserve_liquidate_partial_1d() public {
        CDPVault_TypeA vault = createCDPVault_TypeA(token, 150 ether, 0, 1.25 ether, 0.95 ether, 1 ether, 1.05 ether,0, WAD, int64(uint64(WAD)), 0, 0);
    ...
    function test_reserve_liquidate_full_4() public {
        CDPVault_TypeA vault = createCDPVault_TypeA(token, 150 ether, 10 ether - 1, 1.5 ether, 1 ether, 1 ether, 1.05 ether, 0, WAD, int64(uint64(WAD)), 0, 0);

        // create position
        _depositCash(vault, 15 ether);
        _modifyCollateralAndDebt(vault, 15 ether, 10 ether - 1);

        // liquidate position
        _updateSpot(1.0 ether - 1);
        createCredit(address(this), 5 ether - 1);
        _liquidate(vault, address(this), 10); // 15 ether * price

        assertEq(debt(address(vault)), 0);
        assertEq(debt(address(buffer)), 0);

        assertEq(vault.cash(address(this)), 15 ether);
        assertEq(credit(address(this)), 13); // rounding error
    ...
    function test_non_reserve_liquidate_full() public {
        CDPVault_TypeA vault = createCDPVault_TypeA(token, 0, 0, 1.25 ether, 1 ether, 1 ether, 1.05 ether, 0, WAD, int64(uint64(WAD)), 0, 0);
