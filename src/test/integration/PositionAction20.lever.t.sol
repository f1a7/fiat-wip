...
import {PermitParams} from "../../TransferAction.sol";
import {SwapAction, SwapParams, SwapType, SwapProtocol} from "../../SwapAction.sol";
import {LeverParams, PositionAction} from "../../PositionAction.sol";
...
contract PositionAction20_Lever_Test is IntegrationTestBase {
    ...
    function test_increaseLever_DAI_vault_with_aux_swap_from_WETH() public {
        ...
        uint256 borrowAmount = auxAmountOutMin; // we want the amount of fiat we borrow to be equal to the amount of underliers we swap in
        uint256 amountOutMin = borrowAmount * 98 / 100;

        uint256 expectedAmountOut; // amount out after swaping fiat for collateral token
        ...
        assertEq(fiat.balanceOf(address(positionAction)), 0);
    }
    ...
    function test_increaseLever_DAI_vault_with_aux_swap_from_USDC() public {
        ...
        uint256 borrowAmount = auxAmountOutMin; // we want the amount of fiat we borrow to be equal to the amount of underliers we receieve in aux swap
    ...
    function test_increaseLever_USDT_vault_with_aux_swap_from_DAI() public {
        ...
        uint256 borrowAmount = auxAmountOutMin * 1e12; // borrow the same amount of fiat as the aux swap returns in USDT (so we are levering up 2x)
