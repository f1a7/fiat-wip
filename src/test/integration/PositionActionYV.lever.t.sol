...
import {PositionActionYV} from "../../PositionActionYV.sol";
...
contract PositionActionYV_Lever_Test is IntegrationTestBase {
    ...
    function test_decreaseLever_USDT() public {
        ...
        // assert new collateral amount is the same as initialCollateral minus the amount of collateral we swapped for fiat
        assertEq(collateral, initialCollateral - subCollateral);

        // assert new normalDebt is the same as initialNormalDebt minus the amount of fiat we received from swapping collateral
        assertEq(normalDebt, initialNormalDebt - amountOut);
