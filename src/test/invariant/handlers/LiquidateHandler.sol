...
contract LiquidateHandler is BaseHandler {
    ...
    ) BaseHandler ("LiquidateHandler", testContract_) {
        vault = vault_;
    ...
    function createPosition(uint256 healthFactor) public useTimestamps onlyNonActor("users", msg.sender) {
    ...
    function liquidateRandom(uint256 randomSeed, uint256 liquidationHealthFactor, bool fullLiquidation) public useTimestamps {
        trackCallStart(msg.sig);
    ...
    function liquidateAll() public useTimestamps() {
        trackCallStart(msg.sig);
    ...
    function warp(uint256 warpAmount_) public useTimestamps {
