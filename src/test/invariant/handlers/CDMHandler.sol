...
import {WAD, min} from "../../../utils/Math.sol";

import {BaseHandler, InvariantTestBase} from "./BaseHandler.sol";

import {CDM, ACCOUNT_CONFIG_ROLE, getCreditLine} from "../../../CDM.sol";

contract CDMHandler is BaseHandler {
    CDM public cdm;

    constructor(address cdm_, InvariantTestBase testContract_) BaseHandler("CDMHandler", testContract_) {
        cdm = CDM(cdm_);
    }
    ...
    function setParameter(uint256 debtCeiling) public {
        ...
        if (registered["vaults"][vault]) return;

        addActor("vaults", vault);
    ...
    function modifyBalance(address from, address to, uint256 amount) public {
        ...
        addActors("users", [from, to, msg.sender]);
        addActors("vaults", [from, to, msg.sender]);
