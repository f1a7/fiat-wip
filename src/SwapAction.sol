...
import {abs} from "./utils/Math.sol";
...
contract SwapAction is TransferAction {
    ...
    ) external returns (uint256 retAmount) {
    ...
    function balancerSwap(
        ...
        if (swapType == SwapType.EXACT_IN) {
            ...
            limits[0] = int256(amount); // positive signifies tokens going into the vault from the caller
            limits[pathLength] = -int256(limit); // negative signifies tokens going out of the vault to the caller
        } else {
            kind = SwapKind.GIVEN_OUT;
            amountToApprove = limit;
            limits[0] = -int256(amount);
            limits[pathLength] = int256(limit);
        }
