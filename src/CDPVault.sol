...
// Epoch Constants
// Number of seconds in an epoch [seconds]
uint256 constant EPOCH_DURATION = 3 days;
// Number of epochs that have to pass until an epoch claim can be fixed
uint256 constant EPOCH_FIX_DELAY = 1;
// Number of epochs for which an epoch claim can be fixed
uint256 constant EPOCH_FIX_TIMEOUT = 3;
...
abstract contract CDPVault is AccessControl, Pause, Permission, InterestRateModel, ICDPVaultBase {
    ...
    error CDPVault__checkUtilizationRatio_maxUtilizationRatio();
    ...
    /// @return _ Current epoch
    function getCurrentEpoch() public view returns (uint256) {
        return (block.timestamp - (block.timestamp % EPOCH_DURATION)) / EPOCH_DURATION;
    }
    ...
    function _calculateAssetsAndLiabilities(uint256 totalNonFixedCreditWithheld) internal returns (
        ...
        (globalIRS, accruedInterest) = _calculateGlobalIRS(
            globalIRS, _calculateRateAccumulator(globalIRS), totalNormalDebt_, 0, 0, 0, 0, 0
        );
    ...
    function _fixUndelegationClaims(uint256 epochWithinEpochFixTimeout) internal returns (
        ...
        Epoch[] memory epochsCache = new Epoch[](EPOCH_FIX_TIMEOUT + 1);
        ...
        for (uint256 i; i <= EPOCH_FIX_TIMEOUT; ) {
            ...
            uint256 epoch = currentEpoch - EPOCH_FIX_TIMEOUT + i;
        ...
        if (EPOCH_FIX_TIMEOUT >= requestedEpoch)
            epochWithinEpochFixTimeoutCache_ = epochsCache[EPOCH_FIX_TIMEOUT - requestedEpoch];
    }
    ...
    function claimUndelegatedCredit(uint256 claimForEpoch) external returns (uint256 creditAmount) {
        uint256 currentEpoch = getCurrentEpoch();
        if (currentEpoch < claimForEpoch + EPOCH_FIX_DELAY) revert CDPVault__claimUndelegatedCredit_epochNotClaimable();
    ...
    function _calculateUtilizationRatio(GlobalIRS memory globalIRS) internal view returns(uint256 utilizationRatio) {
        (int256 balance, uint256 debtCeiling) = cdm.accounts(address(this));
        uint256 totalDebt_ = calculateDebt(totalNormalDebt, globalIRS.rateAccumulator, globalIRS.globalAccruedRebate);
        utilizationRatio = (totalDebt_ == 0)
            ? 0 : wdiv(totalDebt_, totalDebt_ + getCreditLine(balance, debtCeiling) - totalAccruedFees);
    }

    /// @notice Calculates the global rate accumulator based on the current global interest rate state
    function _calculateRateAccumulator(GlobalIRS memory globalIRS) internal view returns(uint64) {
        uint64 interestRate;
        if(globalIRS.baseRate < 0){
            ...
            uint256 utilizationRatio = _calculateUtilizationRatio(globalIRS);
        ...
        return _calculateRateAccumulator(globalIRS, interestRate);
    }
    ...
    function virtualIRS(address position) public view override returns (
        ...
        GlobalIRS memory globalIRS = getGlobalIRS();
        rateAccumulator = _calculateRateAccumulator(globalIRS);
        (globalIRS, ) = _calculateGlobalIRS(globalIRS, rateAccumulator, totalNormalDebt, 0, 0, 0, 0, 0);
    ...
    function _updateLimitOrderAndIRS(
        ...
        uint64 rateAccumulator = _calculateRateAccumulator(globalIRS);        
    ...
    function _checkUtilizationRatio(GlobalIRS memory globalIRS) internal view {
        uint256 utilizationRatio = _calculateUtilizationRatio(globalIRS);
        uint256 maxUtilizationRatio = uint64(utilizationParams >> 64);
        if(utilizationRatio > maxUtilizationRatio) {
            revert CDPVault__checkUtilizationRatio_maxUtilizationRatio();
        }
    }
    ...
    function modifyCollateralAndDebt(
        ...
        totalNormalDebt = add(totalNormalDebt_, deltaNormalDebt);
        ...
        if(deltaNormalDebt > 0) _checkUtilizationRatio(globalIRS);
    ...
    /// for which it can be redeemed for FIAT
    ...
    struct ExchangeCache {
        ...
        uint256 spotPrice;
        uint64 rateAccumulator;
        // cached state variables to be evaluated after processing all limit orders
        GlobalIRS globalIRS;
        ...
        uint256 claimedRebate;
        uint256 debtFloor;
    }

    /// @notice Executes a limit order by reducing the associated position's collateral
    /// and debt balances by the credit provided by the taker
    function _executeLimitOrder(
        ExchangeCache memory cache, uint256 limitPriceTick, uint256 limitOrderId
        ...
        // calculate the collateral amount to sell for the normal debt / credit amounts that can be exchanged
        uint256 normalDebtToExchange;
        uint256 collateralToBuy;
        uint256 creditToExchange;
        {
        // calculate the amount of credit to exchange (min(min(debt value, collateral value)), taker credit balance))
        uint256 exchangeRate = wmul(cache.spotPrice, limitPriceTick);
        uint256 debt = calculateDebt(position.normalDebt, cache.rateAccumulator, positionIRS.accruedRebate);
        uint256 creditLeftToExchange = cache.creditToExchange - cache.creditExchanged;
        creditToExchange = min(
            // credit left to exchange
            creditLeftToExchange,
            // max. credit that can be exchanged using this limit order (min(debtValue, collateralValue))
            min(
                ...
                (debt > creditLeftToExchange && debt - creditLeftToExchange < cache.debtFloor)
                    ? debt - cache.debtFloor : debt,
                wmul(position.collateral, exchangeRate)
            )
        );

        // calculate the rebate amount to claim and deduct it from the accrued rebate
        (cache.claimedRebate, positionIRS.accruedRebate) = _calculateRebateClaim(
            creditToExchange, debt, positionIRS.accruedRebate
        );

        // adjust normalized debt we are able to exchange
        normalDebtToExchange = calculateNormalDebt(
            creditToExchange, cache.rateAccumulator, cache.claimedRebate
        );
        collateralToBuy = wdiv(creditToExchange, exchangeRate);
        }
        ...
        _modifyPosition(
            ...
            position,
            positionIRS,
            -int256(collateralToBuy),
            -int256(normalDebtToExchange),
            totalNormalDebt_
        );

        // update the total normalized debt, total exchanged credit, released collateral and accrued interest amounts 
        cache.totalNormalDebt -= normalDebtToExchange;
        cache.collateralExchanged += collateralToBuy;
        cache.creditExchanged += creditToExchange;
        cache.accruedInterest += accruedInterest;

        emit ExecuteLimitOrder(limitOrderId, limitPriceTick, collateralToBuy, creditToExchange);
    ...
    function exchange(
        ...
        cache.creditToExchange = creditToExchange;
        ...
        while (cache.creditExchanged < creditToExchange) {
            ...
            cache = _executeLimitOrder(cache, limitPriceTick, limitOrderId);
            // get the next oldest limit order from the queue
            // break if we tried executing this limit order already
