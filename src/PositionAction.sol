...
/// @notice Struct containing parameters used for borrowing or repaying FIAT
///         and optionally swapping FIAT to an arbitrary token or vice versa
struct CreditParams {
    ...
    address creditor;
    // optional swap from FIAT to arbitrary token
    SwapParams auxSwap;
}
...
struct LeverParams {
    ...
    // the swap parameters to swap collateral to FIAT or vice versa
...
abstract contract PositionAction is IERC3156FlashBorrower, ICreditFlashBorrower, TransferAction, BaseAction {
    ...
    error PositionAction__delegateViaFiat_InvalidAuxSwap();
    ...
    /// @param swapAmountOut the amount of tokens received from the fiat flash loan swap [CDPVault.tokenScale()]
    ...
    /// @notice Adds debt to a CDP Vault by minting FIAT (and optionally swaps FIAT to an arbitrary token)
    /// @param position The CDP Vault position
    /// @param vault The CDP Vault
    ...
    /// @notice Repays debt to a CDP Vault via FIAT (optionally swapping an arbitrary token to FIAT)
    /// @param position The CDP Vault position
    /// @param vault The CDP Vault
    ...
    /// @notice Swap for fiat or transfer fiat directly, then delegate to a vault
    /// @param creditor The address to transfer fiat or swap tokens from
    /// @param vault The CDP Vault to delegate credit to
    /// @param credit The amount of credit to delegate [wad]
    /// @param swapParams The swap parameters for swapping an arbitrary asset to fiat
    /// @param permitParams The permit parameters
    function delegateViaFiat(
        address creditor,
        address vault,
        uint256 credit,
        ...
        // perform swap from arbitrary token to FIAT
        if (swapParams.assetIn != address(0)) {
            if (swapParams.recipient != address(this)) revert PositionAction__delegateViaFiat_InvalidAuxSwap();
            credit = _transferAndSwap(creditor, swapParams, permitParams);
        } else if (creditor != address(this)) {
            // otherwise just transfer FIAT directly from creditor
            _transferFrom(
                address(fiat),
                creditor,
                address(this),
        ...
        // transfer fiat to credit
        fiat.approve(address(minter), credit);
    ...
    /// @notice Increase the leverage of a position by taking out a flash loan and buying FIAT
    /// @param leverParams The parameters for the lever action,
    ///                    `primarySwap` - parameters to swap FIAT provided by the flash loan into the collateral token
    ...
    /// @param upFrontAmount The amount of `upFrontToken` to transfer to the LeverAction contract [upFrontToken.decimals()]
    ...
    function increaseLever(
        ...
        if (leverParams.primarySwap.swapType != SwapType.EXACT_IN ||
            leverParams.primarySwap.assetIn != address(fiat) ||
    ...
    /// @param leverParams The parameters for the lever action:
    ///                    `primarySwap` swap parameters to swap the collateral withdrawn from the CDPVault using the
    ///                    flash loan to FIAT
    ///                    `auxSwap` swap parameters to swap the collateral not used to payback the flash loan
    ...
    function onFlashLoan(
        ...
        // derive the amount of normal debt from the amount of FIAT swapped
        ...
        // mint fiat to pay back the flash loans
        minter.exit(address(this), leverParams.primarySwap.amount);

        // Approve fiat to be used to pay back the flash loan.
        fiat.approve(address(flashlender), leverParams.primarySwap.amount);
    ...
    function onCreditFlashLoan(
        ...
        // derive the amount of normal debt from the amount of FIAT received from the swap
        ...
        // swap collateral to fiat and calculate the amount leftover
        uint256 residualAmount = withdrawnCollateral - swapAmountIn;

        // mint fiat from collateral to pay back the flash loan
        fiat.approve(address(minter), leverParams.primarySwap.amount);
    ...
    /// @notice Mints FIAT and optionally swaps FIAT to an arbitrary token
    /// @param creditParams The credit parameters
    function _borrow(CreditParams calldata creditParams) internal {
        ...
        if (creditParams.auxSwap.assetIn == address(0)) {
            ...
            // hanlde exit swap
            if (creditParams.auxSwap.assetIn != address(fiat)) revert PositionAction__borrow_InvalidAuxSwap();
            _delegateCall(
                address(swapAction),
                abi.encodeWithSelector(
                    swapAction.swap.selector,
    ...
    /// @notice Repays debt by redeeming FIAT and optionally swaps an arbitrary token to fiat
    /// @param position The CDP Vault position
    /// @param vault The CDP Vault
    /// @param creditParams The credit parameters
    /// @param permitParams The permit parameters
    function _repay(
        ...
        // transfer arbitrary token and swap to fiat
        uint256 amount;
        if (creditParams.auxSwap.assetIn != address(0)) {
            ...
            // calculate the amount of fiat to repay
            ...
            if (creditParams.creditor != address(this)) {
                // transfer fiat directly from creditor
                _transferFrom(
                    address(fiat),
        ...
        // generate credit from fiat to repay with
        fiat.approve(address(minter), amount);
    ...
    /// @notice Compute normal debt and compensate for precision loss.
    /// @param vault The CDP Vault
    /// @param position The CDP Vault position
    /// @param debt The debt to convert to normal debt using `position` in `vault`'s rateAccumulator and accruedRebate [wad]
